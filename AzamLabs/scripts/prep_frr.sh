...
# Content is the image's own /etc/frr/daemons (FRR 10.7.0) verbatim, with ONLY
# the daemon toggles this node needs flipped from =no to =yes. Reconcile against
# `docker run --rm --entrypoint cat <image> /etc/frr/daemons` on any image bump.
cat > "$dir/daemons" <<'FRR_DAEMONS_EOF'
