# PNetLab Artifacts & Releases Verification Report

Last updated: 2026-09-19 00:12:03 UTC

## Step 1: Git Repository Verification

- **Remote URL**: `https://codeberg.org/netkillui/Pnetlabv8.git`
- **Local Clone Path**: [`track-1-git/`](file:///e:/Git/EMULATOR/0-P-UNTOUCHED/track-1-git)
- **Branch**: `main`

## Step 2: Codeberg Package API Releases Verification

- **Package Registry API**: `https://codeberg.org/api/v1/packages/netkillui`
- **Debian APT Repository**: `https://codeberg.org/api/packages/netkillui/debian` (dist: `resolute`, component: `main`)

### Verified File Manifest

| Category | Local Path | Size (bytes) | Status | SHA256 Checksum |
| :--- | :--- | :--- | :--- | :--- |
| Debian (pnetlab-bridge-dkms v6.8.78resolute1) | `debian/pool/resolute/main/pnetlab-bridge-dkms_6.8.78resolute1_all.deb` | 142,698 | ✅ VERIFIED | `8066b5796736d4c1c423be392867cb6142080cf6bfd5f4e4967e12469f7e7cb7` |
| Debian (pnetlab-bridge-dkms v6.8.79resolute1) | `debian/pool/resolute/main/pnetlab-bridge-dkms_6.8.79resolute1_all.deb` | 142,686 | ✅ VERIFIED | `f93c80e78df04ddcd77dcaed417fce5f6982a8f2175a27fa7ef0d49a55f26300` |
| Debian (pnetlab-bridge-dkms v6.8.80resolute1) | `debian/pool/resolute/main/pnetlab-bridge-dkms_6.8.80resolute1_all.deb` | 142,784 | ✅ VERIFIED | `6e29e360ae344dd2cb673b16d29402363672e02f59c78b4ccb60984dfe0dcb3f` |
| Debian (pnetlab-bridge-dkms v6.8.81resolute1) | `debian/pool/resolute/main/pnetlab-bridge-dkms_6.8.81resolute1_all.deb` | 142,828 | ✅ VERIFIED | `a7e0bf1dc88b123635276f2931a15e5bef66f57c52c4417043c54272afc86e84` |
| Debian (pnetlab-docker v6.8.78resolute1) | `debian/pool/resolute/main/pnetlab-docker_6.8.78resolute1_amd64.deb` | 3,618 | ✅ VERIFIED | `ff6609c4bdefcce6c4962a8f25e4b06fa2dbcf8e027b47a6d84f2fa0bfea64a7` |
| Debian (pnetlab-docker v6.8.79resolute1) | `debian/pool/resolute/main/pnetlab-docker_6.8.79resolute1_amd64.deb` | 3,616 | ✅ VERIFIED | `a825c368a569f749f80e93d37216d6e7104133b85c79865c15600ad8deaa049b` |
| Debian (pnetlab-docker v6.8.80resolute1) | `debian/pool/resolute/main/pnetlab-docker_6.8.80resolute1_amd64.deb` | 3,616 | ✅ VERIFIED | `ace5a1826ad06b77dae9dde9afe2de3c3ea7ce743cd04eb3e2600d368e713e23` |
| Debian (pnetlab-docker v6.8.81resolute1) | `debian/pool/resolute/main/pnetlab-docker_6.8.81resolute1_amd64.deb` | 3,616 | ✅ VERIFIED | `3f67f7b6efe6b33e1f3efad138cfebf5247c484d88f6f4bc9f36c84e75b580ba` |
| Debian (pnetlab-guacd v6.8.78resolute1) | `debian/pool/resolute/main/pnetlab-guacd_6.8.78resolute1_amd64.deb` | 463,070 | ✅ VERIFIED | `ba7435f00e015efd828cef8eedfb050fbd20261c0a91f4b8ccd284232b37fe10` |
| Debian (pnetlab-guacd v6.8.79resolute1) | `debian/pool/resolute/main/pnetlab-guacd_6.8.79resolute1_amd64.deb` | 463,132 | ✅ VERIFIED | `8348f84d756273b592ec1ce184c2be573607086d3baf07a66d9cf06e31bd3455` |
| Debian (pnetlab-guacd v6.8.80resolute1) | `debian/pool/resolute/main/pnetlab-guacd_6.8.80resolute1_amd64.deb` | 463,150 | ✅ VERIFIED | `4259ace7332e5e8b54f1419fc9f3c0ec159d7db2741380a139fd0c30584338e5` |
| Debian (pnetlab-guacd v6.8.81resolute1) | `debian/pool/resolute/main/pnetlab-guacd_6.8.81resolute1_amd64.deb` | 463,248 | ✅ VERIFIED | `e6e4a198923148eafca7c83ddb882422829781887a26eae52f62ade8aaea266f` |
| Debian (pnetlab-qemu v6.8.78resolute1) | `debian/pool/resolute/main/pnetlab-qemu_6.8.78resolute1_amd64.deb` | 1,275,462 | ✅ VERIFIED | `382da8190a386e3984855f0aaa96ecb5f32a2ff53993eac824a33434ef4920bc` |
| Debian (pnetlab-qemu v6.8.79resolute1) | `debian/pool/resolute/main/pnetlab-qemu_6.8.79resolute1_amd64.deb` | 1,275,538 | ✅ VERIFIED | `03090de077cd596318b55b3c47eb7f9030beffd7b89a6b59105eecca45ea3a7c` |
| Debian (pnetlab-qemu v6.8.80resolute1) | `debian/pool/resolute/main/pnetlab-qemu_6.8.80resolute1_amd64.deb` | 1,275,566 | ✅ VERIFIED | `5e59014f5a24a03f102084e7180d8ea643245585b6b6a1e7a207983a428800a1` |
| Debian (pnetlab-qemu v6.8.81resolute1) | `debian/pool/resolute/main/pnetlab-qemu_6.8.81resolute1_amd64.deb` | 1,275,598 | ✅ VERIFIED | `2a078f822b6348c2f33869c5db9c19671010b98f7eb18fe43f2927d26fff5b79` |
| Debian (pnetlab-satellite v6.8.78resolute1) | `debian/pool/resolute/main/pnetlab-satellite_6.8.78resolute1_amd64.deb` | 82,378,066 | ✅ VERIFIED | `bc7422cf64f2fdee24302cb3b0a5f170679caf27a64ecc18e98a190432bfd596` |
| Debian (pnetlab-satellite v6.8.79resolute1) | `debian/pool/resolute/main/pnetlab-satellite_6.8.79resolute1_amd64.deb` | 9,981,780 | ✅ VERIFIED | `02fb08c36d030c6b03ef67e2da1ae5563c027605605ec271fa13985f7e4c3870` |
| Debian (pnetlab-satellite v6.8.80resolute1) | `debian/pool/resolute/main/pnetlab-satellite_6.8.80resolute1_amd64.deb` | 9,982,868 | ✅ VERIFIED | `8b7ec963e1ffbd3e811f20ed861ed7537e497588eda8c46975cfd22e185e9ff3` |
| Debian (pnetlab-satellite v6.8.81resolute1) | `debian/pool/resolute/main/pnetlab-satellite_6.8.81resolute1_amd64.deb` | 9,982,948 | ✅ VERIFIED | `a52dcf8ee800d40d6d042eb2f2d29c944698c53973bc67e37bb3eba2acf4d0dc` |
| Debian (pnetlab-schema v6.8.78resolute1) | `debian/pool/resolute/main/pnetlab-schema_6.8.78resolute1_amd64.deb` | 9,066 | ✅ VERIFIED | `8b55771279f3cfbe132b658ca331ca9d40161021456f7a435f153f80a85825fc` |
| Debian (pnetlab-schema v6.8.79resolute1) | `debian/pool/resolute/main/pnetlab-schema_6.8.79resolute1_amd64.deb` | 9,104 | ✅ VERIFIED | `be1c7cd96670e664ce48b3ac8a1b7cf31e33a9db9163080d2060630369ff3527` |
| Debian (pnetlab-schema v6.8.80resolute1) | `debian/pool/resolute/main/pnetlab-schema_6.8.80resolute1_amd64.deb` | 9,184 | ✅ VERIFIED | `f705a9d0f8cdc9974fce130d144867110a032bf1f10329d54c15f452ee3fd356` |
| Debian (pnetlab-schema v6.8.81resolute1) | `debian/pool/resolute/main/pnetlab-schema_6.8.81resolute1_amd64.deb` | 9,204 | ✅ VERIFIED | `152e8a676aad3e9ba48ddc2f2f261fee0988ecbeda26174690d6dc6ddbffd31f` |
| Debian (pnetlab-vpcs v6.8.78resolute1) | `debian/pool/resolute/main/pnetlab-vpcs_6.8.78resolute1_amd64.deb` | 90,288 | ✅ VERIFIED | `4fff6ca5c74c11a09566e6c4d7ada2c81bf847f2daf38d33b7a83a57f8131734` |
| Debian (pnetlab-vpcs v6.8.79resolute1) | `debian/pool/resolute/main/pnetlab-vpcs_6.8.79resolute1_amd64.deb` | 90,324 | ✅ VERIFIED | `6d986a6b4c8cedfb38fdc7d8a9305f87b7bf5c26dd6eb9ffdd6dac1c270dc214` |
| Debian (pnetlab-vpcs v6.8.80resolute1) | `debian/pool/resolute/main/pnetlab-vpcs_6.8.80resolute1_amd64.deb` | 90,386 | ✅ VERIFIED | `4bd717e3ef14266be4f9d890ec337479526301c7362f4c13abc7e6a2678f2d0b` |
| Debian (pnetlab-vpcs v6.8.81resolute1) | `debian/pool/resolute/main/pnetlab-vpcs_6.8.81resolute1_amd64.deb` | 90,426 | ✅ VERIFIED | `3ef31c9917bc58d555ee09013c4a824981db794c3f2295aaddbda48b2b816a56` |
| Debian (pnetlab v6.8.78resolute1) | `debian/pool/resolute/main/pnetlab_6.8.78resolute1_amd64.deb` | 87,935,844 | ✅ VERIFIED | `088426541ac429a19e9417655b7570773d55c776caa7fd6afc0c209d88af974d` |
| Debian (pnetlab v6.8.79resolute1) | `debian/pool/resolute/main/pnetlab_6.8.79resolute1_amd64.deb` | 15,536,812 | ✅ VERIFIED | `543ec692ef4b5e95520424bf03c9eaebbdb6fc5cbe19e861db12a9e182a3274e` |
| Debian (pnetlab v6.8.80resolute1) | `debian/pool/resolute/main/pnetlab_6.8.80resolute1_amd64.deb` | 15,536,528 | ✅ VERIFIED | `9e606f4c38ecf57491fe2599e278821e022e0d44a4eb1512ba7b38b0b593076a` |
| Debian (pnetlab v6.8.81resolute1) | `debian/pool/resolute/main/pnetlab_6.8.81resolute1_amd64.deb` | 15,537,618 | ✅ VERIFIED | `3c694717a453c6566268970831a90f99be408f561bf5c272792f5a1a4fb59b7a` |
| Generic (pnetlab-core-assets @ 0.channel) | `generic/0.channel/pnetlab-latest.json` | 205 | ✅ VERIFIED | `84313e16a7fdf44afa446a9daada81d99168d91fcb12d869406098f36ae086d4` |
| Generic (pnetlab-core-assets @ 0.channel) | `generic/0.channel/pnetlab-latest.json.sha256` | 86 | ✅ VERIFIED | `4e89dda53cde0fca11fb1fb88eb95be1311633700feca9de9b4845d3faf7ac5e` |
| Generic (pnetlab-core-assets @ 0.channel) | `generic/0.channel/pnetlab-latest.json.sig` | 566 | ✅ VERIFIED | `f584ea0cd21e80b60d72f31854e5a5e0679eaea1b5940a4bebd5303a245417a1` |
| Generic (pnetlab-core-assets @ 0.channel) | `generic/0.channel/pnetlab-network-install-latest.sh` | 176,628 | ✅ VERIFIED | `b6b3c8b0e81f1a2ac2b2d1af37f4261ec76d5bfd8e9c98045390137efb74f37a` |
| Generic (pnetlab-core-assets @ 0.channel) | `generic/0.channel/pnetlab-network-install-latest.sh.sha256` | 100 | ✅ VERIFIED | `c09eccbaf0b0e4c922e4fc38a0675cd47a98f0771ba257ef56bb77bb66c13dd2` |
| Generic (pnetlab-core-assets @ 6.8.78resolute1) | `generic/6.8.78resolute1/pnetlab-6.8.78resolute1-manifest.json` | 1,513 | ✅ VERIFIED | `289945b0ea1e6d364d5c892f17945ec4ac5e042b57b65724d250329b5950f8c3` |
| Generic (pnetlab-core-assets @ 6.8.78resolute1) | `generic/6.8.78resolute1/pnetlab-6.8.78resolute1-manifest.json.sha256` | 104 | ✅ VERIFIED | `9116ee0c92b6b887d1f0702cbdfe909b366200f88b2391623f04facfa8e09d1d` |
| Generic (pnetlab-core-assets @ 6.8.78resolute1) | `generic/6.8.78resolute1/pnetlab-6.8.78resolute1-manifest.json.sig` | 566 | ✅ VERIFIED | `c9edd0841291e89d552bba94f52cebcbac697f6888a85bd34fa0d0e8a20c0f93` |
| Generic (pnetlab-core-assets @ 6.8.78resolute1) | `generic/6.8.78resolute1/pnetlab-core-assets-6.8.78resolute1.tar.zst` | 75,116,436 | ✅ VERIFIED | `b36bc01d72c33e69d97122e37f25eaf58476ae7d1af442a9473c1f42a2a1ad9d` |
| Generic (pnetlab-core-assets @ 6.8.78resolute1) | `generic/6.8.78resolute1/pnetlab-core-assets-6.8.78resolute1.tar.zst.sha256` | 110 | ✅ VERIFIED | `8fa2dfefa428f23f9870208fbe63ff85f1d32904de13225a7873c6c4e61251f2` |
| Generic (pnetlab-core-assets @ 6.8.78resolute1) | `generic/6.8.78resolute1/pnetlab-core-assets-6.8.78resolute1.tar.zst.sig` | 566 | ✅ VERIFIED | `f12e2ba56387cd912a7f9f39366311111d2e9f86f80e7f4c58985620ffcfc147` |
| Generic (pnetlab-core-assets @ 6.8.78resolute1) | `generic/6.8.78resolute1/pnetlab-install-resolute-satellite-6.8.78resolute1.sh` | 15,185 | ✅ VERIFIED | `6e6469593a7a42519939b076cfed55ea3017fc107e38db0d7080d96674e4c751` |
| Generic (pnetlab-core-assets @ 6.8.78resolute1) | `generic/6.8.78resolute1/pnetlab-install-resolute-satellite-6.8.78resolute1.sh.sha256` | 120 | ✅ VERIFIED | `0c5f90aaa405fcf7ce10f0b4c4deb9bb8874a5d5f18d44ed84c9e64668e778f7` |
| Generic (pnetlab-core-assets @ 6.8.78resolute1) | `generic/6.8.78resolute1/pnetlab-network-install-6.8.78resolute1.sh` | 174,477 | ✅ VERIFIED | `6f9b3c24d4975a60e3cc64aa11cc0c60d83699e4938bee340953c52549242262` |
| Generic (pnetlab-core-assets @ 6.8.78resolute1) | `generic/6.8.78resolute1/pnetlab-network-install-6.8.78resolute1.sh.sha256` | 109 | ✅ VERIFIED | `d5ed2df532044db23eabe96e603d77cf8840db86da70a02f9da73d2412b2f89f` |
| Generic (pnetlab-core-assets @ 6.8.79resolute1) | `generic/6.8.79resolute1/pnetlab-6.8.79resolute1-manifest.json` | 1,826 | ✅ VERIFIED | `bbf5879a4506d78f458cdfb9b8560075b6b65822515e546fab31b0bdc3371629` |
| Generic (pnetlab-core-assets @ 6.8.79resolute1) | `generic/6.8.79resolute1/pnetlab-6.8.79resolute1-manifest.json.sha256` | 104 | ✅ VERIFIED | `fd7c469cc3b9b2c12c19567fddf68e49ffa2293ee1d6f445667c9182bd7bd75e` |
| Generic (pnetlab-core-assets @ 6.8.79resolute1) | `generic/6.8.79resolute1/pnetlab-6.8.79resolute1-manifest.json.sig` | 566 | ✅ VERIFIED | `91f21ce160d3ed5a793f8dab8ef5c133b0d6d429177746d3067e8031c0ab3197` |
| Generic (pnetlab-core-assets @ 6.8.79resolute1) | `generic/6.8.79resolute1/pnetlab-core-assets-6.8.79resolute1.tar.zst` | 75,116,436 | ✅ VERIFIED | `b36bc01d72c33e69d97122e37f25eaf58476ae7d1af442a9473c1f42a2a1ad9d` |
| Generic (pnetlab-core-assets @ 6.8.79resolute1) | `generic/6.8.79resolute1/pnetlab-core-assets-6.8.79resolute1.tar.zst.sha256` | 110 | ✅ VERIFIED | `3a5dbb5a347a9f5e07263ee40a0a8017fe7afeea3eb52b4581ed1754700cb43a` |
| Generic (pnetlab-core-assets @ 6.8.79resolute1) | `generic/6.8.79resolute1/pnetlab-core-assets-6.8.79resolute1.tar.zst.sig` | 566 | ✅ VERIFIED | `691e7b08153f8dc1972ad9e9af03535aa97021c20e2eb6497b3357a4938b2326` |
| Generic (pnetlab-core-assets @ 6.8.79resolute1) | `generic/6.8.79resolute1/pnetlab-install-resolute-satellite-6.8.79resolute1.sh` | 15,185 | ✅ VERIFIED | `6e6469593a7a42519939b076cfed55ea3017fc107e38db0d7080d96674e4c751` |
| Generic (pnetlab-core-assets @ 6.8.79resolute1) | `generic/6.8.79resolute1/pnetlab-install-resolute-satellite-6.8.79resolute1.sh.sha256` | 120 | ✅ VERIFIED | `c99831f9cf7c4e35bd3028c01aadc38c170d34adeebbe4f4a510d822976875e5` |
| Generic (pnetlab-core-assets @ 6.8.79resolute1) | `generic/6.8.79resolute1/pnetlab-network-install-6.8.79resolute1.sh` | 176,628 | ✅ VERIFIED | `b6b3c8b0e81f1a2ac2b2d1af37f4261ec76d5bfd8e9c98045390137efb74f37a` |
| Generic (pnetlab-core-assets @ 6.8.79resolute1) | `generic/6.8.79resolute1/pnetlab-network-install-6.8.79resolute1.sh.sha256` | 109 | ✅ VERIFIED | `4f4cd8d193f939cf0e6e37481c2d964b517f47085f45e54364b25c39ade436db` |
| Generic (pnetlab-core-assets @ 6.8.80resolute1) | `generic/6.8.80resolute1/pnetlab-6.8.80resolute1-manifest.json` | 1,826 | ✅ VERIFIED | `f2cb7e54c4bc803d5a9d19e8556a4be3e805f76faed732b69ae4f0d892b46207` |
| Generic (pnetlab-core-assets @ 6.8.80resolute1) | `generic/6.8.80resolute1/pnetlab-6.8.80resolute1-manifest.json.sha256` | 104 | ✅ VERIFIED | `7a381625d425ddf0eb51f861eafdfd4e2ddb9b6ebe1082c6c8b74012fb39e91c` |
| Generic (pnetlab-core-assets @ 6.8.80resolute1) | `generic/6.8.80resolute1/pnetlab-6.8.80resolute1-manifest.json.sig` | 566 | ✅ VERIFIED | `aeaded2a8d77dd718e5fdd5728284dbb4ae450f9f903cd9fc9e8049ed65aea72` |
| Generic (pnetlab-core-assets @ 6.8.80resolute1) | `generic/6.8.80resolute1/pnetlab-core-assets-6.8.80resolute1.tar.zst` | 75,116,436 | ✅ VERIFIED | `b36bc01d72c33e69d97122e37f25eaf58476ae7d1af442a9473c1f42a2a1ad9d` |
| Generic (pnetlab-core-assets @ 6.8.80resolute1) | `generic/6.8.80resolute1/pnetlab-core-assets-6.8.80resolute1.tar.zst.sha256` | 110 | ✅ VERIFIED | `283b7c9a7d9696297405e27b7f3aff780832b72f16d7cde2424c834f48cd421e` |
| Generic (pnetlab-core-assets @ 6.8.80resolute1) | `generic/6.8.80resolute1/pnetlab-core-assets-6.8.80resolute1.tar.zst.sig` | 566 | ✅ VERIFIED | `d8ddade415116cc30c277d2642c936fa8c9a4f97078709a77c62297f1593955e` |
| Generic (pnetlab-core-assets @ 6.8.80resolute1) | `generic/6.8.80resolute1/pnetlab-install-resolute-satellite-6.8.80resolute1.sh` | 15,266 | ✅ VERIFIED | `59c676866bf9b49afc79187017dc92f786698e8311173f922c8b36848eccec71` |
| Generic (pnetlab-core-assets @ 6.8.80resolute1) | `generic/6.8.80resolute1/pnetlab-install-resolute-satellite-6.8.80resolute1.sh.sha256` | 120 | ✅ VERIFIED | `af450569754f3a07b662eba5169a1fe020364075443197ccb40b745749d3a33f` |
| Generic (pnetlab-core-assets @ 6.8.80resolute1) | `generic/6.8.80resolute1/pnetlab-network-install-6.8.80resolute1.sh` | 176,628 | ✅ VERIFIED | `b6b3c8b0e81f1a2ac2b2d1af37f4261ec76d5bfd8e9c98045390137efb74f37a` |
| Generic (pnetlab-core-assets @ 6.8.80resolute1) | `generic/6.8.80resolute1/pnetlab-network-install-6.8.80resolute1.sh.sha256` | 109 | ✅ VERIFIED | `6ee809f986094c3d0276cea9876ac28e2cf2c127957771ac4b3070d378fd37f2` |
| Generic (pnetlab-core-assets @ 6.8.81resolute1) | `generic/6.8.81resolute1/pnetlab-6.8.81resolute1-manifest.json` | 1,826 | ✅ VERIFIED | `0e1a2ec4a0c7f203f2739ab07db5ac2dda0296dc798b3b71287b97195d016366` |
| Generic (pnetlab-core-assets @ 6.8.81resolute1) | `generic/6.8.81resolute1/pnetlab-6.8.81resolute1-manifest.json.sha256` | 104 | ✅ VERIFIED | `c01610fbd895db04f77c28e74b38e2de31a2225e4e81bc3b62358b5d8d89a994` |
| Generic (pnetlab-core-assets @ 6.8.81resolute1) | `generic/6.8.81resolute1/pnetlab-6.8.81resolute1-manifest.json.sig` | 566 | ✅ VERIFIED | `32aeaf3a9c63e332e178ee21a21aee01ef660dc56d21fb32b2c752c618660ad4` |
| Generic (pnetlab-core-assets @ 6.8.81resolute1) | `generic/6.8.81resolute1/pnetlab-core-assets-6.8.81resolute1.tar.zst` | 75,116,412 | ✅ VERIFIED | `2e75df594dc5cf4e5dd9e4017f0ea59387c2a7e08ce4896d80c241a80ae2571d` |
| Generic (pnetlab-core-assets @ 6.8.81resolute1) | `generic/6.8.81resolute1/pnetlab-core-assets-6.8.81resolute1.tar.zst.sha256` | 110 | ✅ VERIFIED | `1a95fe413edac54ecff382649e60d3d13bd9fce456232fa4e5231264994f6ddd` |
| Generic (pnetlab-core-assets @ 6.8.81resolute1) | `generic/6.8.81resolute1/pnetlab-core-assets-6.8.81resolute1.tar.zst.sig` | 566 | ✅ VERIFIED | `c26efa25b38cb1478a9cea18cd2601e23e83a847e7103268e1156f816ae3150d` |
| Generic (pnetlab-core-assets @ 6.8.81resolute1) | `generic/6.8.81resolute1/pnetlab-install-resolute-satellite-6.8.81resolute1.sh` | 15,266 | ✅ VERIFIED | `59c676866bf9b49afc79187017dc92f786698e8311173f922c8b36848eccec71` |
| Generic (pnetlab-core-assets @ 6.8.81resolute1) | `generic/6.8.81resolute1/pnetlab-install-resolute-satellite-6.8.81resolute1.sh.sha256` | 120 | ✅ VERIFIED | `87fd3189fae223c7c46acf660179e82e52c7a79c9afc4488f84dcff637d979dd` |
| Generic (pnetlab-core-assets @ 6.8.81resolute1) | `generic/6.8.81resolute1/pnetlab-network-install-6.8.81resolute1.sh` | 176,628 | ✅ VERIFIED | `b6b3c8b0e81f1a2ac2b2d1af37f4261ec76d5bfd8e9c98045390137efb74f37a` |
| Generic (pnetlab-core-assets @ 6.8.81resolute1) | `generic/6.8.81resolute1/pnetlab-network-install-6.8.81resolute1.sh.sha256` | 109 | ✅ VERIFIED | `96f06aa3dab2146d4d0bc013c3fe4f6095570236a7b06c5493dfe7358c723c00` |
| Metadata Index | `metadata/binary-all-Packages` | 7,311 | ✅ VERIFIED | `1a2f95c9fa434e70b410387ea1c96cc70429278695c0f5f22c1e96489772d238` |
| Metadata Index | `metadata/binary-amd64-Packages` | 55,710 | ✅ VERIFIED | `d955f204915f8933cb9a37670e5d38ecd663490499bfc8cef82699a6cb505bdf` |
| Metadata Index | `metadata/dists-resolute-Release` | 2,637 | ✅ VERIFIED | `841df8ce675f5bb8624d0662661f99216428f377f17f41fcd8368732d6e533a3` |
| Metadata Index | `metadata/generic-package-details.json` | 22,233 | ✅ VERIFIED | `49b80a0ee3ca6b0483198a94f64b976afa09af711d2b0ab0805efb6e81446187` |
| Metadata Index | `metadata/packages-api-response.json` | 70,689 | ✅ VERIFIED | `6adfd8b06a9f928a2a48c2f111f986049b6a5fd116ef93c3d11fd96a6406b2f8` |
